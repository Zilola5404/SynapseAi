...
export function validateOrderRisk(input: OrderRiskCheckInput): OrderRiskCheckResult {
  ...
  // 2b. Max Drawdown Check
  const peak = peakEquityUsdt && peakEquityUsdt > 0 ? peakEquityUsdt : accountEquity;
  const equityNow = currentEquityUsdt ?? accountEquity;
  if (peak > 0 && equityNow < peak) {
    const drawdownPct = ((peak - equityNow) / peak) * 100;
    if (drawdownPct >= riskSettings.maxDrawdownPct) {
      return {
        allowed: false,
        reason: `Превышена максимальная просадка (${drawdownPct.toFixed(2)}% / макс. ${riskSettings.maxDrawdownPct}%). Новые ордера заблокированы.`,
      };
    }
  }
