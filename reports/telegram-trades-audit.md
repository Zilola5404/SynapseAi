...
Rows were **not deleted**. Telegram now labels them so they are not mixed with strategy stats:

| Label | Meaning |
|---|---|
| 🧪 TEST TRADE | `/testorder` certification (`TEST_ORDER`) — exclude from Strategy Statistics |
| 🤖 AUTO TRADE | Scanner / auto path |
| 👤 MANUAL TRADE | User confirmed a signal |
...
### 3) −$0.62 — 🧪 TEST TRADE (TESTNET TEST_ORDER)
...
### 4) −$0.47 — 🧪 TEST TRADE leftover after TP ladder
